...
const (
	...
	defaultValidatorPrefixPath = "meta-chain-"
...
// generatembftChainConfig creates and persists mbft chain configuration to the provided file path
func (p *genesisParams) generateMBftChainConfig(o command.OutputFormatter) error {
	...
	mBftConfig := &mbft.MBFTConfig{
		...
		RewardConfig: &mbft.RewardsConfig{
	...
	chainConfig := &chain.Chain{
		...
		Params: &chain.Params{
			...
			Engine: map[string]interface{}{
				string(server.MBFTConsensus): mBftConfig,
			},
		},
	...
	allocs, err := p.deployContracts(rewardTokenByteCode, mBftConfig, chainConfig, burnContractAddr)
	if err != nil {
		return err
	}
	...
	genesisExtraData, err := GenerateExtraDataMBft(validatorMetadata)
	if err != nil {
		return err
	}
...
func (p *genesisParams) deployContracts(
	...
	if rewardTokenByteCode != nil {
		...
		// to RewardTokenContract address and update mbft config
