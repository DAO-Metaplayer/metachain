...
import (
	...
	mbftWallet "github.com/DAO-Metaplayer/metachain/consensus/mbft/wallet"
...
func GetRootchainID() (string, error) {
	...
	for _, c := range containers {
		if c.Labels["chain-type"] == "rootchain" {
			return c.ID, nil
		}
	}
...
func GetECDSAKey(privateKey, accountDir, accountConfig string) (ethgo.Key, error) {
	...
	return mbftWallet.GetEcdsaFromSecret(secretsManager)
}
...
func GetValidatorInfo(validatorAddr ethgo.Address, supernetManagerAddr, stakeManagerAddr types.Address,
	chainID int64, txRelayer txrelayer.TxRelayer) (*mbft.ValidatorInfo, error) {
	...
	validatorInfo := &mbft.ValidatorInfo{
