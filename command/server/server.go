...
import (
	...
	"github.com/DAO-Metaplayer/metachain/command/server/config"
	"github.com/DAO-Metaplayer/metachain/command/server/export"
...
func setFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVar(
		...
		"start the state sync relayer service (mbft only)",
	)
...
func setLegacyFlags(cmd *cobra.Command) {
	// Legacy METABFT base timeout flag
	cmd.Flags().Uint64Var(
		&params.metabftBaseTimeoutLegacy,
		metabftBaseTimeoutFlagLEGACY,
		0,
		"",
	)

	_ = cmd.Flags().MarkHidden(metabftBaseTimeoutFlagLEGACY)
}
