...
import (
	...
	"github.com/DAO-Metaplayer/metachain/merkle-tree"
...
type checkpointManager struct {
	...
	// consensusBackend is abstraction for mbft consensus specific functions
	consensusBackend mbftBackend
...
func newCheckpointManager(key ethgo.Key, checkpointOffset uint64,
	...
	blockchain blockchainBackend, backend mbftBackend, logger hclog.Logger,
...
func (c *checkpointManager) submitCheckpoint(latestHeader *types.Header, isEndOfEpoch bool) error {
	...
	if initialBlockNumber < latestHeader.Number {
		...
		parentExtra, err = GetmetabftExtra(parentHeader.ExtraData)
		if err != nil {
			return err
		}
	}
	...
	for blockNumber := initialBlockNumber + 1; blockNumber <= latestHeader.Number; blockNumber++ {
		...
		currentExtra, err = GetmetabftExtra(currentHeader.ExtraData)
		if err != nil {
			return err
		}
	...
	if currentExtra == nil {
		...
		currentExtra, err = GetmetabftExtra(latestHeader.ExtraData)
		if err != nil {
			return err
		}
	}
...
func parseExitEvent(h *types.Header, l *ethgo.Log) (*ExitEvent, bool, error) {
	extra, err := GetmetabftExtra(h.ExtraData)
	if err != nil {
		return nil, false,
