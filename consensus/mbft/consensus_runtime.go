...
type runtimeConfig struct {
	MBFTConfig            *MBFTConfig
	DataDir               string
	Key                   *wallet.Key
	State                 *State
	blockchain            blockchainBackend
	mbftBackend           mbftBackend
...
func (c *consensusRuntime) initStateSyncManager(logger hcf.Logger) error {
	if c.IsBridgeEnabled() {
		stateSenderAddr := c.config.MBFTConfig.Bridge.StateSenderAddr
		stateSyncManager := newStateSyncManager(
			...
			&stateSyncConfig{
				...
				stateSenderStartBlock:    c.config.MBFTConfig.Bridge.EventTrackerStartBlocks[stateSenderAddr],
				jsonrpcAddr:              c.config.MBFTConfig.Bridge.JSONRPCEndpoint,
				dataDir:                  c.config.DataDir,
				topic:                    c.config.bridgeTopic,
				maxCommitmentSize:        maxCommitmentSize,
				numBlockConfirmations:    c.config.numBlockConfirmations,
				blockTrackerPollInterval: c.config.MBFTConfig.BlockTrackerPollInterval.Duration,
			},
			c,
		)
...
func (c *consensusRuntime) initCheckpointManager(logger hcf.Logger) error {
	if c.IsBridgeEnabled() {
		...
		txRelayer, err := txrelayer.NewTxRelayer(
			txrelayer.WithIPAddress(c.config.MBFTConfig.Bridge.JSONRPCEndpoint),
		...
		c.checkpointManager = newCheckpointManager(
			...
			c.config.MBFTConfig.Bridge.CheckpointManagerAddr,
			txRelayer,
			c.config.blockchain,
			c.config.mbftBackend,
...
func (c *consensusRuntime) initStakeManager(logger hcf.Logger) error {
	rootRelayer, err := txrelayer.NewTxRelayer(txrelayer.WithIPAddress(c.config.MBFTConfig.Bridge.JSONRPCEndpoint))
	if err != nil {
		return err
	}
	...
	c.stakeManager, err = newStakeManager(
		...
		c.config.MBFTConfig.Bridge.CustomSupernetManagerAddr,
		c.config.blockchain,
		c.config.mbftBackend,
		int(c.config.MBFTConfig.MaxValidatorSetSize),
	)
...
func (c *consensusRuntime) IsBridgeEnabled() bool {
	return c.config.MBFTConfig.IsBridgeEnabled()
}
...
func (c *consensusRuntime) FSM() error {
	...
	blockBuilder, err := c.config.blockchain.NewBlockBuilder(
		...
		c.config.txPool,
		c.config.MBFTConfig.BlockTime.Duration,
		c.logger,
	)
	...
	ff := &fsm{
		config:            c.config.MBFTConfig,
		parent:            parent,
		backend:           c.config.blockchain,
		mbftBackend:       c.config.mbftBackend,
...
func (c *consensusRuntime) restartEpoch(header *types.Header) (*epochMetadata, error) {
	...
	validatorSet, err := c.config.mbftBackend.GetValidators(header.Number, nil)
	if err != nil {
...
func (c *consensusRuntime) calculateCommitEpochInput(
	...
	*contractsapi.DistributeRewardForRewardPoolFn, error) {
	...
	blockExtra, err := GetmetabftExtra(currentBlock.ExtraData)
	if err != nil {
		return nil, nil, err
	}
	...
	if blockHeader.Number > commitEpochLookbackSize {
		for i := 0; i < commitEpochLookbackSize; i++ {
			validators, err := c.config.mbftBackend.GetValidators(blockHeader.Number-2, nil)
			if err != nil {
				return nil, nil, err
			}
...
func (c *consensusRuntime) isFixedSizeOfEpochMet(blockNumber uint64, epoch *epochMetadata) bool {
	return epoch.FirstBlockInEpoch+c.config.MBFTConfig.EpochSize-1 == blockNumber
}
...
func (c *consensusRuntime) isFixedSizeOfSprintMet(blockNumber uint64, epoch *epochMetadata) bool {
	return (blockNumber-epoch.FirstBlockInEpoch+1)%c.config.MBFTConfig.SprintSize == 0
}
...
func (c *consensusRuntime) IsValidValidator(msg *proto.Message) bool {
	...
	if c.fsm == nil {
		c.logger.Warn("unable to validate METABFT message sender, because FSM is not initialized")

		return false
	}

	if err := c.fsm.ValidateSender(msg); err != nil {
		c.logger.Error("invalid METABFT message received", "error", err)
...
func (c *consensusRuntime) IsValidProposalHash(proposal *proto.Proposal, hash []byte) bool {
	...
	extra, err := GetmetabftExtra(block.Header.ExtraData)
	if err != nil {
...
func (c *consensusRuntime) BuildPrePrepareMessage(
	...
	message, err := c.config.Key.SignMETABFTMessage(&msg)
	if err != nil {
...
func (c *consensusRuntime) BuildRoundChangeMessage(
	...
	signedMsg, err := c.config.Key.SignMETABFTMessage(&msg)
	if err != nil {
		c.logger.Error("Cannot sign message", "Error", err)
...
func (c *consensusRuntime) getFirstBlockOfEpoch(epochNumber uint64, latestHeader *types.Header) (uint64, error) {
	...
	blockExtra, err := GetmetabftExtra(latestHeader.ExtraData)
	if err != nil {
		return 0, err
	}
