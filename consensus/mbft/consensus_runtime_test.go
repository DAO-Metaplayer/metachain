...
func TestConsensusRuntime_isFixedSizeOfEpochMet_NotReachedEnd(t *testing.T) {
	...
	for _, c := range cases {
		runtime.config.MBFTConfig.EpochSize = c.epochSize
...
func TestConsensusRuntime_isFixedSizeOfSprintMet_NotReachedEnd(t *testing.T) {
	...
	for _, c := range cases {
		runtime.config.MBFTConfig.SprintSize = c.sprintSize
		runtime.epoch.FirstBlockInEpoch = c.firstBlockInEpoch
		assert.False(t,
...
func TestConsensusRuntime_OnBlockInserted_EndOfEpoch(t *testing.T) {
	...
	mbftBackendMock := new(mbftBackendMock)
	mbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validatorSet).Times(3)
...
func TestConsensusRuntime_OnBlockInserted_MiddleOfEpoch(t *testing.T) {
	...
	mbftBackendMock := new(mbftBackendMock)
	mbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(nil).Once()
	...
	config := &runtimeConfig{
		MBFTConfig: &MBFTConfig{EpochSize: epochSize},
		blockchain: blockchainMock,
		txPool:     txPool,
	}
...
func Test_NewConsensusRuntime(t *testing.T) {
	...
	mbftBackendMock := new(mbftBackendMock)
	mbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators).Times(3)
	...
	assert.Equal(t, uint64(10), runtime.config.MBFTConfig.SprintSize)
	assert.Equal(t, uint64(10), runtime.config.MBFTConfig.EpochSize)
	assert.Equal(t, "0x0000000000000000000000000000000000000101", contracts.ValidatorSetContract.String())
	assert.Equal(t, "0x1300000000000000000000000000000000000000", runtime.config.MBFTConfig.Bridge.StateSenderAddr.String())
	assert.Equal(t, "0x1000000000000000000000000000000000000000", runtime.config.MBFTConfig.Bridge.CheckpointManagerAddr.String())
	assert.True(t, runtime.IsBridgeEnabled())
	systemStateMock.AssertExpectations(t)
	blockchainMock.AssertExpectations(t)
	mbftBackendMock.AssertExpectations(t)
}
...
func TestConsensusRuntime_calculateCommitEpochInput_SecondEpoch(t *testing.T) {
	...
	mbftBackendMock := new(mbftBackendMock)
	mbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators.GetPublicIdentities()).Twice()
...
func TestConsensusRuntime_IsValidValidator_BasicCases(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			msg, err := signer.Key().SignMETABFTMessage(&proto.Message{From: sender.Address().Bytes()})
...
func TestConsensusRuntime_TamperMessageContent(t *testing.T) {
	...
	msg, err = sender.Key().SignMETABFTMessage(msg)
	assert.NoError(t, err)
...
func TestConsensusRuntime_BuildRoundChangeMessage(t *testing.T) {
	...
	signedMsg, err := key.SignMETABFTMessage(&expected)
	require.NoError(t, err)
