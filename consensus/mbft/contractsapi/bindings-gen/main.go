...
import (
	...
	gensc "github.com/DAO-Metaplayer/metachain/consensus/mbft/contractsapi"
...
`
	...
	if err = common.SaveFileSafe("./consensus/mbft/contractsapi/contractsapi.go", output, 0600); err != nil {
		log.Fatal(err)
	}
}
...
func generateType(generatedData *generatedData, name string, obj *abi.Type, res *[]string) (string, error) {
	...
	for _, tupleElem := range obj.TupleElems() {
		...
		// uint8 internally in mbft, we can use regexp to replace those values with the
