...
// mbftMixDigest represents a hash of "MBFT Mix" to identify whether the block is from mbft consensus engine
var MBFTMixDigest = types.StringToHash("adce6e5230abe012342a44e4e9b6d05997d6f015387ae0e59be924afc7ec70c1")
...
func (i *Extra) ValidateFinalizedData(header *types.Header, parent *types.Header, parents []*types.Header,
	chainID uint64, consensusBackend mbftBackend, domain []byte, logger hclog.Logger) error {
	...
	parentExtra, err := GetmetabftExtra(parent.ExtraData)
	if err != nil {
...
func (i *Extra) ValidateParentSignatures(blockNumber uint64, consensusBackend mbftBackend, parents []*types.Header,
...
// GetmetabftExtraClean returns unmarshaled extra field from the passed in header,
// but without signatures for the given header (it only includes signatures for the parent block)
func GetmetabftExtraClean(extraRaw []byte) ([]byte, error) {
	extra, err := GetmetabftExtra(extraRaw)
	if err != nil {
		return nil, err
	}
	...
	return metabftExtra.MarshalRLPTo(nil), nil
}

// GetmetabftExtra returns the istanbul extra data field from the passed in header
func GetmetabftExtra(extraRaw []byte) (*Extra, error) {
