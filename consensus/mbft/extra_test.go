...
func TestExtra_ValidateFinalizedData_UnhappyPath(t *testing.T) {
	...
	polyBackendMock := new(mbftBackendMock)
...
func TestExtra_InitGenesisValidatorsDelta(t *testing.T) {
	...
	t.Run("Happy path", func(t *testing.T) {
		...
		genesisExtra, err := GetmetabftExtra(genesis.ExtraData)
		assert.NoError(t, err)
	...
	t.Run("Invalid Extra data", func(t *testing.T) {
		...
		_, err := GetmetabftExtra(genesis.ExtraData)

		require.Error(t, err)
	})
}

func Test_GetmetabftExtraClean(t *testing.T) {
	t.Parallel()
	...
	extraClean, err := GetmetabftExtraClean(extra.MarshalRLPTo(nil))
	require.NoError(t, err)
...
func Test_GetmetabftExtraClean_Fail(t *testing.T) {
	t.Parallel()
	...
	extra, err := GetmetabftExtraClean(append(randomBytes[:], []byte{0x12, 0x6}...))
	require.Error(t, err)
	require.Nil(t, extra)
}
