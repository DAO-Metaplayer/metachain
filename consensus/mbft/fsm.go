...
type fsm struct {
	// mbft consensus protocol configuration
	config *MBFTConfig
	...
	// mbftBackend implements methods needed from the mbft
...
func (f *fsm) BuildProposal(currentRound uint64) ([]byte, error) {
	...
	extraParent, err := GetmetabftExtra(parent.ExtraData)
	if err != nil {
		return nil, err
	}
...
func (f *fsm) Validate(proposal []byte) error {
	...
	parentExtra, err := GetmetabftExtra(f.parent.ExtraData)
	if err != nil {
		return err
	}
	...
	if err := extra.ValidateParentSignatures(block.Number(), f.mbftBackend, nil, f.parent, parentExtra,
	...
	if f.logger.IsTrace() && block.Number() > 1 {
		validators, err := f.mbftBackend.GetValidators(block.Number()-2, nil)
		if err != nil {
...
func (f *fsm) Insert(proposal []byte, committedSeals []*messages.CommittedSeal) (*types.FullBlock, error) {
	...
	extra, err := GetmetabftExtra(newBlock.Block.Header.ExtraData)
	if err != nil {
...
func validateHeaderFields(parent *types.Header, header *types.Header, blockTimeDrift uint64) error {
	...
	// verify mix digest
	if header.MixHash != MBFTMixDigest {
