...
func TestFSM_BuildProposal_WithoutCommitEpochTxGood(t *testing.T) {
	...
	fsm := &fsm{parent: parent, blockBuilder: mBlockBuilder, config: &MBFTConfig{}, backend: blockchainMock,
...
func TestFSM_BuildProposal_EpochEndingBlock_ValidatorsDeltaExists(t *testing.T) {
	...
	blockExtra, err := GetmetabftExtra(stateBlock.Block.Header.ExtraData)
	assert.NoError(t, err)
...
func TestFSM_BuildProposal_NonEpochEndingBlock_ValidatorsDeltaNil(t *testing.T) {
	...
	fsm := &fsm{parent: parent, blockBuilder: blockBuilderMock,
		config: &MBFTConfig{}, backend: &blockchainMock{},
...
func TestFSM_Validate_ExitEventRootNotExpected(t *testing.T) {
	...
	mbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators.GetPublicIdentities(), nil).Once()
	...
	fsm := &fsm{
		...
		config:            &MBFTConfig{BlockTimeDrift: 1},
...
func TestFSM_Validate_FailToVerifySignatures(t *testing.T) {
	...
	mbftBackendMock := new(mbftBackendMock)
	mbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validatorsMetadata, nil).Once()
