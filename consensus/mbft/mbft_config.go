...
const ConsensusName = "mbft"

// mbftConfig is the configuration file for the MBft consensus protocol.
type MBFTConfig struct {
...
// LoadmbftConfig loads chain config from provided path and unmarshals mbftConfig
func LoadMBFTConfig(chainConfigFile string) (MBFTConfig, error) {
	chainCfg, err := chain.ImportFromFile(chainConfigFile)
	if err != nil {
		return MBFTConfig{}, err
	}

	mbftConfig, err := GetMBFTConfig(chainCfg)
	if err != nil {
		return MBFTConfig{}, err
	}
...
// GetMBFTConfig deserializes provided chain config and returns MBFTConfig
func GetMBFTConfig(chainConfig *chain.Chain) (MBFTConfig, error) {
	consensusConfigJSON, err := json.Marshal(chainConfig.Params.Engine[ConsensusName])
	if err != nil {
		return MBFTConfig{}, err
	}

	var mBFTConfig MBFTConfig
	if err = json.Unmarshal(consensusConfigJSON, &mBFTConfig); err != nil {
		return MBFTConfig{}, err
	}

	return mBFTConfig, nil
}
...
func (p *MBFTConfig) IsBridgeEnabled() bool {
	return p.Bridge != nil
}
...
// TokenConfig is the configuration of native token used by chain network
