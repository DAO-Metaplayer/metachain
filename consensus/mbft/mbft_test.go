...
// the test initializes mbft and chain mock (map of headers) after which a new header is verified
...
func TestMBFT_VerifyHeader(t *testing.T) {
	t.Parallel()
	...
	for i := uint64(1); i < mBftConfig.EpochSize; i++ {
	...
	// create mbft with appropriate mocks
	mbft := &Mbft{
	...
	currentHeader := &types.Header{
		Number:     mBftConfig.EpochSize + 1,
		ParentHash: parentHeader.Hash,
		Timestamp:  parentHeader.Timestamp + 1,
		MixHash:    MBFTMixDigest,
		Difficulty: 1,
	}
	...
	assert.ErrorContains(t, mbft.VerifyHeader(currentHeader), "invalid header hash")
	...
	assert.ErrorContains(t, mbft.VerifyHeader(currentHeader), "failed to verify signatures for parent of block")
	...
	assert.NoError(t, mbft.VerifyHeader(currentHeader))
	...
	mbft.validatorsCache = newValidatorsSnapshotCache(hclog.NewNullLogger(), newTestState(t), blockchainMock)
	assert.NoError(t, mbft.validatorsCache.storeSnapshot(&validatorSnapshot{Epoch: 0, Snapshot: validatorSetCurrent})) // invalid validator set is submitted
	assert.NoError(t, mbft.validatorsCache.storeSnapshot(&validatorSnapshot{Epoch: 1, Snapshot: validatorSetCurrent}))
	assert.ErrorContains(t, mbft.VerifyHeader(currentHeader), "failed to verify signatures for parent of block")

	// clean validators cache again and set valid snapshots
	mbft.validatorsCache = newValidatorsSnapshotCache(hclog.NewNullLogger(), newTestState(t), blockchainMock)
	assert.NoError(t, mbft.validatorsCache.storeSnapshot(&validatorSnapshot{Epoch: 0, Snapshot: validatorSetParent}))
	assert.NoError(t, mbft.validatorsCache.storeSnapshot(&validatorSnapshot{Epoch: 1, Snapshot: validatorSetCurrent}))
	assert.NoError(t, mbft.VerifyHeader(currentHeader))
...
func TestMbft_Close(t *testing.T) {
	t.Parallel()
	...
	mbft.closeCh = make(chan struct{})

	assert.Error(t, errExpected, mbft.Close())
...
func TestMbft_GetSyncProgression(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(t, result, mbft.GetSyncProgression())
}
...
func Test_Factory(t *testing.T) {
	...
	assert.Equal(t, txPool, mbft.txPool)
	assert.Equal(t, epochSize, mbft.consensusConfig.EpochSize)
	assert.Equal(t, params, mbft.config)
}
