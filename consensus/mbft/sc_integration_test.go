...
import (
	...
	"github.com/DAO-Metaplayer/metachain/helper/hex"
...
func TestIntegration_CommitEpoch(t *testing.T) {
	...
	for _, currentValidators := range validatorSets {
		...
		// validator data for mbft config
		...
		for i, val := range accSet {
			...
			// create validator data for mbft config
		...
		// init ValidatorSet
		err := initValidatorSet(mBFTConfig, transition)
		require.NoError(t, err)

		// init RewardPool
		err = initRewardPool(mBFTConfig, transition)
		require.NoError(t, err)

		// approve reward pool as reward token spender
		err = approveRewardPoolAsSpender(mBFTConfig, transition)
		require.NoError(t, err)

		// create input for commit epoch
		commitEpoch := createTestCommitEpochInput(t, 1, mBFTConfig.EpochSize)
		...
		distributeRewards := createTestDistributeRewardsInput(t, 1, accSet, mBFTConfig.EpochSize)
		...
		commitEpoch = createTestCommitEpochInput(t, 2, mBFTConfig.EpochSize)
		input, err = commitEpoch.EncodeAbi()
		require.NoError(t, err)
		...
		distributeRewards = createTestDistributeRewardsInput(t, 2, accSet, mBFTConfig.EpochSize)
		input, err = distributeRewards.EncodeAbi()
		require.NoError(t, err)
