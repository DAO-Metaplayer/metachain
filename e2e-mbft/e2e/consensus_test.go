...
func TestE2E_Consensus_RegisterValidator(t *testing.T) {
	...
	mbftConfig, err := mbft.LoadMBFTConfig(path.Join(cluster.Config.TmpDir, chainConfigFileName))
	require.NoError(t, err)
	...
	_, err = mbft.GetmetabftExtra(genesisBlock.ExtraData)
	require.NoError(t, err)
	...
	require.NoError(t, firstValidator.RegisterValidator(mbftConfig.Bridge.CustomSupernetManagerAddr))

	// register the second validator without stake
	require.NoError(t, secondValidator.RegisterValidator(mbftConfig.Bridge.CustomSupernetManagerAddr))

	// stake manually for the first validator
	require.NoError(t, firstValidator.Stake(mbftConfig, initialStake))

	// stake manually for the second validator
	require.NoError(t, secondValidator.Stake(mbftConfig, initialStake))

	firstValidatorInfo, err := sidechain.GetValidatorInfo(firstValidatorAddr,
		mbftConfig.Bridge.CustomSupernetManagerAddr, mbftConfig.Bridge.StakeManagerAddr,
		mbftConfig.SupernetID, rootChainRelayer, childChainRelayer)
	require.NoError(t, err)
	...
	require.NoError(t, cluster.WaitForBlock(mbftConfig.EpochSize*4, time.Minute))

	checkpointManagerAddr := ethgo.Address(mbftConfig.Bridge.CheckpointManagerAddr)

	// check if the validators are added to active validator set
	rootchainValidators := []*mbft.ValidatorInfo{}
	...
	require.NoError(t, cluster.WaitForBlock(currentBlock.Number+(mbftConfig.EpochSize*2), time.Minute))
...
func TestE2E_Consensus_Validator_Unstake(t *testing.T) {
	...
	require.NoError(t, cluster.WaitForBlock(mbftCfg.EpochSize*3, time.Minute))

	validatorInfo, err := sidechain.GetValidatorInfo(validatorAddr,
		mbftCfg.Bridge.CustomSupernetManagerAddr, mbftCfg.Bridge.StakeManagerAddr,
		mbftCfg.SupernetID, rootChainRelayer, childChainRelayer)
	require.NoError(t, err)
	...
	require.NoError(t, cluster.WaitForBlock(mbftCfg.EpochSize*4, time.Minute))
	...
	require.NoError(t, waitForRootchainEpoch(currentEpoch, time.Minute,
		rootChainRelayer, mbftCfg.Bridge.CheckpointManagerAddr))

	exitEventID := uint64(1)

	// send exit transaction to exit helper
	err = cluster.Bridge.SendExitTransaction(mbftCfg.Bridge.ExitHelperAddr, exitEventID, srv.JSONRPCAddr())
	require.NoError(t, err)
...
func TestE2E_Consensus_EIP1559Check(t *testing.T) {
	...
	cluster := framework.NewTestCluster(t, 5,
		...
		framework.WithBurnContract(&mbft.BurnContractInfo{BlockNumber: 0, Address: types.ZeroAddress}),
	)
	defer cluster.Stop()
