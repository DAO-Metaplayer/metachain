...
const nativeTokenMintableTestCfg = "Mintable chain Coin:MEC:18:true:%s"

// getCheckpointManagerValidators queries rootchain validator set on CheckpointManager contract
func getCheckpointManagerValidators(relayer txrelayer.TxRelayer, checkpointManagerAddr ethgo.Address) ([]*mbft.ValidatorInfo, error) {
	...
	validators := make([]*mbft.ValidatorInfo, validatorsCount)
	...
	for i := 0; i < int(validatorsCount); i++ {
		...
		validators[i] = &mbft.ValidatorInfo{
