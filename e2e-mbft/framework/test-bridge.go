...
import (
	...
	bridgeCommon "github.com/DAO-Metaplayer/metachain/command/bridge/common"
...
func (t *TestBridge) deployRootchainContracts(genesisPath string) error {
	mbftConfig, err := mbft.LoadMBFTConfig(genesisPath)
	if err != nil {
		return err
	}
	...
	args := []string{
		"rootchain",
		"deploy",
		"--stake-manager", mbftConfig.Bridge.StakeManagerAddr.String(),
		"--stake-token", mbftConfig.Bridge.StakeTokenAddr.String(),
...
func (t *TestBridge) fundRootchainValidators(mbftConfig mbft.MBFTConfig) error {
	...
	balances := make([]*big.Int, len(mbftConfig.InitialValidatorSet))
	...
	if err := t.FundValidators(mbftConfig.Bridge.StakeTokenAddr,
...
func (t *TestBridge) whitelistValidators(validatorAddresses []types.Address,
	mbftConfig mbft.MBFTConfig) error {
	...
	args := []string{
		...
		"--supernet-manager", mbftConfig.Bridge.CustomSupernetManagerAddr.String(),
...
func (t *TestBridge) registerGenesisValidators(mbftConfig mbft.MBFTConfig) error {
	validatorSecrets, err := genesis.GetValidatorKeyFiles(t.clusterConfig.TmpDir, t.clusterConfig.ValidatorPrefix)
	if err != nil {
		...
		g.Go(func() error {
			...
			default:
				args := []string{
					...
					"--jsonrpc", t.JSONRPCAddr(),
					"--supernet-manager", mbftConfig.Bridge.CustomSupernetManagerAddr.String(),
					"--" + mbftsecrets.AccountDirFlag, path.Join(t.clusterConfig.TmpDir, secret),
				}
...
func (t *TestBridge) initialStakingOfGenesisValidators(mbftConfig mbft.MBFTConfig) error {
	validatorSecrets, err := genesis.GetValidatorKeyFiles(t.clusterConfig.TmpDir, t.clusterConfig.ValidatorPrefix)
	if err != nil {
		...
		g.Go(func() error {
			...
			default:
				args := []string{
					...
					"--supernet-id", strconv.FormatInt(mbftConfig.SupernetID, 10),
					"--stake-token", mbftConfig.Bridge.StakeTokenAddr.String(),
				}
...
func (t *TestBridge) finalizeGenesis(genesisPath string, mbftConfig mbft.MBFTConfig) error {
	args := []string{
		"mbft",
		"supernet",
		"--jsonrpc", t.JSONRPCAddr(),
...
func (t *TestBridge) FundValidators(tokenAddress types.Address, secretsPaths []string, amounts []*big.Int) error {
	...
	for i := 0; i < len(secretsPaths); i++ {
		secretsManager, err := mbftsecrets.GetSecretsManager(secretsPaths[i], "", true)
		if err != nil {
			return err
		}
