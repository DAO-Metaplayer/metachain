...
type TestClusterConfig struct {
	...
	BurnContract         *mbft.BurnContractInfo
...
func WithBurnContract(burnContract *mbft.BurnContractInfo) ClusterOption {
	return func(h *TestClusterConfig) {
...
func NewTestCluster(t *testing.T, validatorsCount int, opts ...ClusterOption) *TestCluster {
	...
	config.TmpDir, err = os.MkdirTemp("/tmp", "e2e-mbft-")
	...
	err = cluster.Bridge.fundRootchainValidators(mbftConfig)
	require.NoError(t, err)

	// whitelist genesis validators on the rootchain
	err = cluster.Bridge.whitelistValidators(addresses, mbftConfig)
	require.NoError(t, err)

	// register genesis validators on the rootchain
	err = cluster.Bridge.registerGenesisValidators(mbftConfig)
	require.NoError(t, err)

	// do initial staking for genesis validators on the rootchain
	err = cluster.Bridge.initialStakingOfGenesisValidators(mbftConfig)
	require.NoError(t, err)

	// finalize genesis validators on the rootchain
	err = cluster.Bridge.finalizeGenesis(genesisPath, mbftConfig)
	require.NoError(t, err)
...
// RunChainCommand - calls a command line chain function
func RunChainCommand(args []string, stdout io.Writer) error {
...
func (c *TestCluster) InitSecrets(prefix string, count int) ([]types.Address, error) {
	...
	stdOut := c.Config.GetStdout("mbft-secrets", &b)
