...
func NewTestServer(t *testing.T, clusterConfig *TestClusterConfig,
	...
	if config.DataDir == "" {
		dataDir, err := ioutil.TempDir("/tmp", "chain-e2e-")
		require.NoError(t, err)

		config.DataDir = dataDir
	}

	secretsManager, err := mbftsecrets.GetSecretsManager(config.DataDir, "", true)
	require.NoError(t, err)
...
func (t *TestServer) Start() {
	...
	args := []string{
		"server",
		// add data dir
		"--" + mbftsecrets.AccountDirFlag, config.DataDir,
		// add custom chain
		"--chain", config.Chain,
		// enable p2p port
...
func (t *TestServer) Stake(mbftConfig mbft.MBFTConfig, amount *big.Int) error {
	args := []string{
		"mbft",
		"stake",
		"--jsonrpc", t.BridgeJSONRPCAddr(),
		"--stake-manager", mbftConfig.Bridge.StakeManagerAddr.String(),
		"--" + mbftsecrets.AccountDirFlag, t.config.DataDir,
...
func (t *TestServer) RegisterValidator(supernetManagerAddr types.Address) error {
	args := []string{
		...
		"--" + mbftsecrets.AccountDirFlag, t.DataDir(),
	}
...
func (t *TestServer) HasValidatorSealed(firstBlock, lastBlock uint64, validators validator.AccountSet,
	...
	for i := firstBlock + 1; i <= lastBlock; i++ {
		...
		extra, err := mbft.GetmetabftExtra(block.ExtraData)
		if err != nil {
			return false, err
		}
