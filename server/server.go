...
import (
	...
	"github.com/DAO-Metaplayer/metachain/blockchain/storage"
	"github.com/DAO-Metaplayer/metachain/blockchain/storage/leveldb"
	"github.com/DAO-Metaplayer/metachain/blockchain/storage/memory"
	consensusMBFT "github.com/DAO-Metaplayer/metachain/consensus/mbft"
	"github.com/DAO-Metaplayer/metachain/forkmanager"
	"github.com/DAO-Metaplayer/metachain/gasprice"

	"github.com/DAO-Metaplayer/metachain/archive"
	"github.com/DAO-Metaplayer/metachain/blockchain"
	"github.com/DAO-Metaplayer/metachain/chain"
	"github.com/DAO-Metaplayer/metachain/consensus"
	"github.com/DAO-Metaplayer/metachain/consensus/mbft/statesyncrelayer"
	"github.com/DAO-Metaplayer/metachain/consensus/mbft/wallet"
	"github.com/DAO-Metaplayer/metachain/contracts"
	"github.com/DAO-Metaplayer/metachain/crypto"
	"github.com/DAO-Metaplayer/metachain/helper/common"
	"github.com/DAO-Metaplayer/metachain/helper/progress"
	"github.com/DAO-Metaplayer/metachain/jsonrpc"
	...
	itrie "github.com/DAO-Metaplayer/metachain/state/immutable-trie"
	"github.com/DAO-Metaplayer/metachain/state/runtime"
	"github.com/DAO-Metaplayer/metachain/state/runtime/addresslist"
	"github.com/DAO-Metaplayer/metachain/state/runtime/tracer"
	"github.com/DAO-Metaplayer/metachain/txpool"
	"github.com/DAO-Metaplayer/metachain/types"
	"github.com/DAO-Metaplayer/metachain/validate"
...
type Server struct {
	...
	// stateSyncRelayer is handling state syncs execution (mbft exclusive)
...
func NewServer(config *Config) (*Server, error) {
	...
	if ConsensusType(engineName) == MBFTConsensus {
		mBFTConfig, err := consensusMBFT.GetMBFTConfig(config.Chain)
		if err != nil {
			return nil, err
		}

		if mBFTConfig.InitialTrieRoot != types.ZeroHash {
			checkedInitialTrieRoot, err := itrie.HashChecker(mBFTConfig.InitialTrieRoot.Bytes(), stateStorage)
			if err != nil {
				return nil, fmt.Errorf("error on state root verification %w", err)
			}

			if checkedInitialTrieRoot != mBFTConfig.InitialTrieRoot {
				return nil, errors.New("invalid initial state root")
			}

			logger.Info("Initial state root checked and correct")

			initialStateRoot = mBFTConfig.InitialTrieRoot
		}
	}
...
func (s *Server) setupRelayer() error {
	...
	mBFTConfig, err := consensusMBFT.GetMBFTConfig(s.config.Chain)
	if err != nil {
		return fmt.Errorf("failed to extract mbft config: %w", err)
	}

	trackerStartBlockConfig := map[types.Address]uint64{}
	if mBFTConfig.Bridge != nil {
		trackerStartBlockConfig = mBFTConfig.Bridge.EventTrackerStartBlocks
	}
...
func initForkManager(engineName string, config *chain.Chain) error {
	...
	for name, f := range *config.Params.Forks {
		// check if fork is not supported by current chain version
