...
import (
	...
	commonHelpers "github.com/DAO-Metaplayer/metachain/helper/common"
...
func (f *FullNode) setChain(idx byte, e Node) {
	if idx == 16 {
		f.value = e
	} else {
		f.children[idx] = e
...
func (f *FullNode) getChain(idx byte) Node {
	if idx == 16 {
		return f.value
	} else {
...
func (t *Txn) lookup(node interface{}, key []byte) (Node, []byte) {
	...
	case *FullNode:
		...
		child, res := t.lookup(n.getChain(key[0]), key[1:])
...
func (t *Txn) insert(node Node, search, value []byte) Node {
	...
	case *ShortNode:
		...
		if plen == len(n.key) {
			...
			if len(n.key) > plen+1 {
				b.setChain(n.key[plen], &ShortNode{key: n.key[plen+1:], child: n.child})
			} else {
				b.setChain(n.key[plen], n.child)
			}
...
func (t *Txn) delete(node Node, search []byte) (Node, bool) {
	...
	case *FullNode:
		...
		key := search[0]
		newChild, ok := t.delete(n.getChain(key), search[1:])

		if !ok {
			return nil, false
		}

		n.setChain(key, newChild)
